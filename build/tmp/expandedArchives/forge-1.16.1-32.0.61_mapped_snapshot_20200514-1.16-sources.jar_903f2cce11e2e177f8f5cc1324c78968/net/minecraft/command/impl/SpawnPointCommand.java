...
public class SpawnPointCommand {
   ...
   private static int setSpawnPoint(CommandSource source, Collection<ServerPlayerEntity> targets, BlockPos pos) {
      ...
      for(ServerPlayerEntity serverplayerentity : targets) {
         serverplayerentity.func_241153_a_(registrykey, pos, true, false);
      }

      String s = registrykey.func_240901_a_().toString();
      if (targets.size() == 1) {
         source.sendFeedback(new TranslationTextComponent("commands.spawnpoint.success.single", pos.getX(), pos.getY(), pos.getZ(), s, targets.iterator().next().getDisplayName()), true);
      } else {
         source.sendFeedback(new TranslationTextComponent("commands.spawnpoint.success.multiple", pos.getX(), pos.getY(), pos.getZ(), s, targets.size()), true);
      }
