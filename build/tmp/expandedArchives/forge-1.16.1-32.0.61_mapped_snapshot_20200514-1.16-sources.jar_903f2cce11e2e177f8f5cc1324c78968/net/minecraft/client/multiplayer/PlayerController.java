...
public class PlayerController {
   ...
   public void acknowledgePlayerDiggingReceived(ClientWorld worldIn, BlockPos pos, BlockState blockIn, CPlayerDiggingPacket.Action action, boolean successful) {
      ...
      if ((posandrotation == null || !successful || action != CPlayerDiggingPacket.Action.START_DESTROY_BLOCK && blockstate != blockIn) && blockstate != blockIn) {
         worldIn.invalidateRegionAndSetBlock(pos, blockIn);
         if (posandrotation != null) {
            Vector3d vector3d = posandrotation.getPos();
            this.mc.player.setPositionAndRotation(vector3d.x, vector3d.y, vector3d.z, posandrotation.getYaw(), posandrotation.getPitch());
         }
      }
