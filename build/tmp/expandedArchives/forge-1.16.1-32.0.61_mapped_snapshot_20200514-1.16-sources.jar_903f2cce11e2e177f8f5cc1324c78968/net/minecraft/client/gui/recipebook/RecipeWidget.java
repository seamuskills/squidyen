...
public class RecipeWidget extends Widget {
   ...
   public List<ITextProperties> getToolTipText(Screen p_191772_1_) {
      ItemStack itemstack = this.getOrderedRecipes().get(this.currentIndex).getRecipeOutput();
      List<ITextProperties> list = Lists.newArrayList(p_191772_1_.func_231151_a_(itemstack));
      if (this.list.getRecipes(this.book.isFilteringCraftable(this.field_203401_p)).size() > 1) {
         list.add(new TranslationTextComponent("gui.recipebook.moreRecipes"));
      }
