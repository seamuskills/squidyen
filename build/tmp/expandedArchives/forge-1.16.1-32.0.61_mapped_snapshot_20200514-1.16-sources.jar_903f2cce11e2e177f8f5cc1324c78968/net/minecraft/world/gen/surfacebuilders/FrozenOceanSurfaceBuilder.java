   ...
   public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, SurfaceBuilderConfig config) {
      ...
      for(int j1 = Math.max(startHeight, (int)d0 + 1); j1 >= 0; --j1) {
         ...
         if (blockstate1.isAir()) {
            ...
            if (blockstate1.isIn(Blocks.PACKED_ICE) && k <= l && j1 > i1) {
            ...
            --j;
            chunkIn.setBlockState(blockpos$mutable, blockstate2, false);
            if (j == 0 && blockstate2.isIn(Blocks.SAND) && l1 > 1) {
               j = random.nextInt(4) + Math.max(0, j1 - 63);
               blockstate2 = blockstate2.isIn(Blocks.RED_SAND) ? Blocks.RED_SANDSTONE.getDefaultState() : Blocks.SANDSTONE.getDefaultState();
            }
         }
      }
