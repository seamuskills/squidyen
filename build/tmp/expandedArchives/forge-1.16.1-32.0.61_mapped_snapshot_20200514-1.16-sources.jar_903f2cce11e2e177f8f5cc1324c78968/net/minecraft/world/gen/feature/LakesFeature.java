   ...
   public boolean func_230362_a_(ISeedReader p_230362_1_, StructureManager p_230362_2_, ChunkGenerator p_230362_3_, Random p_230362_4_, BlockPos p_230362_5_, BlockStateFeatureConfig p_230362_6_) {
      ...
      if (p_230362_5_.getY() <= 4) {
         ...
         if (p_230362_2_.func_235011_a_(SectionPos.from(p_230362_5_), Structure.field_236381_q_).findAny().isPresent()) {
               ...
               for(int j3 = 0; j3 < 16; ++j3) {
                  for(int j4 = 4; j4 < 8; ++j4) {
                     if (aboolean[(i2 * 16 + j3) * 8 + j4]) {
                        BlockPos blockpos = p_230362_5_.add(i2, j4 - 1, j3);
                        if (isDirt(p_230362_1_.getBlockState(blockpos).getBlock()) && p_230362_1_.getLightFor(LightType.SKY, p_230362_5_.add(i2, j4, j3)) > 0) {
                           Biome biome = p_230362_1_.getBiome(blockpos);
                           if (biome.getSurfaceBuilderConfig().getTop().isIn(Blocks.MYCELIUM)) {
                              p_230362_1_.setBlockState(blockpos, Blocks.MYCELIUM.getDefaultState(), 2);
                           } else {
                              p_230362_1_.setBlockState(blockpos, Blocks.GRASS_BLOCK.getDefaultState(), 2);
                           }
                        }
                     }
                  }
            ...
            if (p_230362_6_.state.getMaterial() == Material.LAVA) {
               for(int j2 = 0; j2 < 16; ++j2) {
                  for(int k3 = 0; k3 < 16; ++k3) {
                     for(int k4 = 0; k4 < 8; ++k4) {
                        boolean flag1 = !aboolean[(j2 * 16 + k3) * 8 + k4] && (j2 < 15 && aboolean[((j2 + 1) * 16 + k3) * 8 + k4] || j2 > 0 && aboolean[((j2 - 1) * 16 + k3) * 8 + k4] || k3 < 15 && aboolean[(j2 * 16 + k3 + 1) * 8 + k4] || k3 > 0 && aboolean[(j2 * 16 + (k3 - 1)) * 8 + k4] || k4 < 7 && aboolean[(j2 * 16 + k3) * 8 + k4 + 1] || k4 > 0 && aboolean[(j2 * 16 + k3) * 8 + (k4 - 1)]);
                        if (flag1 && (k4 < 4 || p_230362_4_.nextInt(2) != 0) && p_230362_1_.getBlockState(p_230362_5_.add(j2, k4, k3)).getMaterial().isSolid()) {
                           p_230362_1_.setBlockState(p_230362_5_.add(j2, k4, k3), Blocks.STONE.getDefaultState(), 2);
                        }
                     }
                  }
               }
            ...
            if (p_230362_6_.state.getMaterial() == Material.WATER) {
               for(int k2 = 0; k2 < 16; ++k2) {
                  for(int l3 = 0; l3 < 16; ++l3) {
                     int l4 = 4;
                     BlockPos blockpos1 = p_230362_5_.add(k2, 4, l3);
                     if (p_230362_1_.getBiome(blockpos1).doesWaterFreeze(p_230362_1_, blockpos1, false)) {
                        p_230362_1_.setBlockState(blockpos1, Blocks.ICE.getDefaultState(), 2);
                     }
                  }
               }
            }
